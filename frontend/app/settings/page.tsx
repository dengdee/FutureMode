...
export default function SettingsPage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!name.trim()) { setError("請輸入顯示名稱。"); return; } setSaving(true); setError(""); setNotice(""); try { const user = await updateCurrentUser({ display_name: name.trim(), email: email.trim() || undefined }); setName(user.display_name ?? name.trim()); setEmail(user.email ?? email.trim()); window.dispatchEvent(new Event("proximate:profile-updated")); setNotice("個人資料已儲存。"); } catch (cause) { setError(cause instanceof Error ? cause.message : "更新失敗，請稍後再試。"); } finally { setSaving(false); } }
