...
export default function SettingsPage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!name.trim()) { setError("請輸入顯示名稱。"); return; } setSaving(true); setError(""); setNotice(""); try { const user = await updateCurrentUser({ display_name: name.trim(), email: email.trim() || undefined }); const savedName = user.display_name ?? name.trim(); setName(savedName); setEmail(user.email ?? email.trim()); window.localStorage.setItem("proximate:profile-name", savedName); window.dispatchEvent(new CustomEvent("proximate:profile-updated", { detail: savedName })); setNotice("個人資料已儲存，側邊欄名稱已同步更新。"); } catch (cause) { setError(cause instanceof Error ? cause.message : "更新失敗，請稍後再試。"); } finally { setSaving(false); } }
