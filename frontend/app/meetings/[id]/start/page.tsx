...
import { endMeeting, getMeeting, startMeeting } from "../../../../lib/api/meetings";
...
export default function StartMeetingPage() {
  ...
  async function beginMeeting() {
    ...
    try { await startMeeting(id); router.push(`/meetings/${id}/live`); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "無法開始會議。"); setBusy(false); }
  }

  async function finishMeeting() {
    setBusy(true); setError("");
    try { const updated = await endMeeting(id); setMeeting(updated); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "無法結束會議。"); }
