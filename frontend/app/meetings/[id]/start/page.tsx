...
import { hasMeetingAudioCaptureIntent, isMeetingAudioCapturing, MeetingAudioCapture, stopMeetingAudioCapture } from "../../../../components/meeting-audio-capture";
...
export default function StartMeetingPage() {
  ...
  const [audioStateVersion, setAudioStateVersion] = useState(0);

  useEffect(() => {
    const refreshAudioState = () => setAudioStateVersion((version) => version + 1);
    window.addEventListener("meeting-audio-start", refreshAudioState);
    window.addEventListener("meeting-audio-stop", refreshAudioState);
    return () => {
      window.removeEventListener("meeting-audio-start", refreshAudioState);
      window.removeEventListener("meeting-audio-stop", refreshAudioState);
    };
  }, []);
  ...
  void audioStateVersion;
  const audioCapturing = isMeetingAudioCapturing(id) || hasMeetingAudioCaptureIntent(id);
  ...
  return <AppShell>
    ...
    <section className="mt-6 rounded-2xl border border-[#cde5df] bg-[#f0fbf8] p-5 sm:p-7">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        {meeting.status !== "completed" && meeting.status !== "cancelled" && <button type="button" onClick={() => void finishMeeting()} disabled={ending || (meeting.status !== "in_progress" && !audioCapturing)} title={meeting.status !== "in_progress" && !audioCapturing ? "請先進入即時會議或開始收音" : undefined} className="inline-flex items-center gap-2 rounded-lg border border-rose-200 bg-white px-4 py-2.5 text-sm font-semibold text-rose-700 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50"><IconPlayerStop size={16} />{ending ? "結束中…" : "結束會議"}</button>}
      </div>
