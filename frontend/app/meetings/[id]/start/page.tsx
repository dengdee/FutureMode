...
export default function StartMeetingPage() {
  ...
  const meetingTimeReached = !meeting.scheduled_at || new Date(meeting.scheduled_at).getTime() <= Date.now();
  ...
  return <AppShell>
    ...
    <section className="mt-6 rounded-2xl border border-[#cde5df] bg-[#f0fbf8] p-5 sm:p-7">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        {meetingTimeReached ? <button type="button" onClick={enterLive} className="inline-flex items-center gap-2 rounded-lg bg-[#0f9f8a] px-4 py-2.5 text-sm font-semibold text-white"><IconPlayerPlay size={16} />進入即時會議</button> : <span className="inline-flex cursor-not-allowed items-center gap-2 rounded-lg bg-[#e6e6e3] px-4 py-2.5 text-sm font-semibold text-[#9b9a97]"><IconPlayerPlay size={16} />尚未到會議時間</span>}
