...
function showInvitationError(cause: unknown) {
  if (cause instanceof ApiClientError && cause.status === 401) {
    toast.error("目前登入憑證無法被 API 驗證，請重新登入後再建立邀請。", {
      action: {
        label: "重新登入",
...
export default function WorkspaceMembersPage() {
  ...
  function load() {
    Promise.all([listTeams(), listTeamMembers(workspaceId)])
      .then(async ([teamResult, memberResult]) => {
        const current =
        ...
        try {
          setInvitations(
            current?.role === "admin" ? await listInvitations(workspaceId) : [],
          );
        } catch {
          setInvitations([]);
        }
      ...
      .catch((cause) =>
        toast.error(
          cause instanceof Error ? cause.message : "無法讀取成員資料。",
        ),
      );
  }
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    try {
      await createInvitation(workspaceId, { email: email.trim(), role });
      setEmail("");
      setRole("member");
      toast.success("邀請已建立；對方登入後會在站內看到邀請。");
      load();
    } catch (cause) {
      showInvitationError(cause);
    } finally {
      setBusy(false);
    }
  }
  async function updateRole(member: TeamMember, nextRole: string) {
    try {
      await updateTeamMember(workspaceId, member.user_id, { role: nextRole });
      setMembers((items) =>
        items.map((item) =>
      ...
      toast.error(cause instanceof Error ? cause.message : "更新角色失敗。 ");
    }
  }
  async function remove(member: TeamMember) {
    if (!window.confirm(`確定移除「${member.display_name || "此成員"}」？`))
      return;
    try {
      await removeTeamMember(workspaceId, member.user_id);
      setMembers((items) =>
        items.filter((item) => item.user_id !== member.user_id),
      );
      toast.success("成員已移除。");
    } catch (cause) {
      toast.error(cause instanceof Error ? cause.message : "移除成員失敗。 ");
    }
  }
  async function cancel(invitation: Invitation) {
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "取消邀請失敗。 ");
    }
  }
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-2xl border border-[#e6e6e3] bg-white p-5 sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold">目前成員</h2>
          </div>
          <div className="mt-5 space-y-3">
            {members.map((member) => (
              <article
                key={member.user_id}
                className="flex flex-wrap items-center gap-3 rounded-xl border border-[#ededeb] p-4"
              >
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#e7f7ef] font-semibold text-[#087e6d]">
                  {Array.from(member.display_name || "成")[0]}
                </span>
                ...
                <select
                  ...
                  className="control-primary w-auto py-2 text-xs"
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-xs font-medium text-red-700 hover:bg-red-50"
                >
            ...
            {members.length === 0 && (
              <p className="rounded-xl bg-[#f7f7f5] p-5 text-sm text-[#787774]">
                尚無可顯示成員。
              </p>
            )}
          </div>
        ...
        <aside className="space-y-6">
          <section className="rounded-2xl border border-[#e6e6e3] bg-white p-5">
            <div className="flex items-center gap-2">
              <IconMailPlus className="text-[#0f9f8a]" size={20} />
              <h2 className="font-semibold">邀請成員</h2>
            </div>
            <p className="mt-2 text-sm leading-6 text-[#787774]">
              輸入對方登入用 Email。邀請不會寄信，對方登入後會在 Proximate
              看到它。
            </p>
            <form onSubmit={invite} className="mt-4 space-y-3">
              <label className="block text-sm font-medium">
                ...
                <input
                  ...
                  className="control-primary mt-2"
              ...
              <button
                disabled={busy}
                className="w-full rounded-lg bg-[#0f9f8a] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"
              >
                {busy ? "建立中…" : "建立邀請"}
              </button>
            </form>
          ...
          <section className="rounded-2xl border border-[#e6e6e3] bg-white p-5">
            <h2 className="font-semibold">待接受邀請</h2>
            <div className="mt-4 space-y-3">
              ...
              {!invitations.some((item) => item.status === "pending") && (
                <p className="text-sm text-[#787774]">目前沒有待接受邀請。</p>
              )}
            </div>
          </section>
        </aside>
