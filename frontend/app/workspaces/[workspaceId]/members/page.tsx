...
export default function WorkspaceMembersPage() {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    try { await createInvitation(workspaceId, { email: email.trim(), role }); setEmail(""); setRole("member"); setMessage("邀請已建立；對方登入後會在站內看到邀請。"); load(); }
    catch (cause) { setMessage(cause instanceof Error ? cause.message : "建立邀請失敗。 "); }
    finally { setBusy(false); }
  }
  async function updateRole(member: TeamMember, nextRole: string) {
    try { await updateTeamMember(workspaceId, member.user_id, { role: nextRole }); setMembers((items) => items.map((item) => item.user_id === member.user_id ? { ...item, role: nextRole } : item)); setMessage("成員角色已更新。"); }
    catch (cause) { setMessage(cause instanceof Error ? cause.message : "更新角色失敗。 "); }
  }
  async function remove(member: TeamMember) {
    if (!window.confirm(`確定移除「${member.display_name || "此成員"}」？`)) return;
    try { await removeTeamMember(workspaceId, member.user_id); setMembers((items) => items.filter((item) => item.user_id !== member.user_id)); setMessage("成員已移除。"); }
    catch (cause) { setMessage(cause instanceof Error ? cause.message : "移除成員失敗。 "); }
  }
  async function cancel(invitation: Invitation) {
    try { await cancelInvitation(workspaceId, invitation.id); setInvitations((items) => items.filter((item) => item.id !== invitation.id)); setMessage("邀請已取消。"); }
    catch (cause) { setMessage(cause instanceof Error ? cause.message : "取消邀請失敗。 "); }
  }
  ...
  return <AppShell><PageHeader eyebrow="Team" title={team ? `${team.name} 的成員` : "成員與邀請"} description="管理團隊中的角色，並以站內邀請加入新成員。" actions={<Link href={`/workspaces/${workspaceId}/meetings`} className="rounded-primary border border-[#d7e8e5] px-4 py-2.5 text-sm font-semibold text-[#087e6d]">查看團隊會議</Link>} />
    ...
    {message && <p role="status" className="mt-5 rounded-xl bg-[#e7f7ef] px-4 py-3 text-sm text-[#087e6d]">{message}</p>}
