...
import { createInvitation, createTeam, listTeams } from "../../lib/api/teams";
...
export default function TeamsPage() {
  ...
  async function submitTeam(event: FormEvent<HTMLFormElement>) {
    ...
    if (inviteRows.some((row) => row.email.trim() && !/^\S+@\S+\.\S+$/.test(row.email.trim()))) {
      setCreateError("請確認每個 Email 格式正確。");
      return;
    }
