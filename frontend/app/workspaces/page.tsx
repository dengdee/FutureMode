...
export default function WorkspacesPage() {
  const requestedTeamId = useSearchParams().get("teamId");
  const { data: authSession } = authClient.useSession();
  function memberName(member: TeamMember) {
    if (member.external_id === authSession?.user?.id) return authSession.user.name || "你";
  ...
  useEffect(() => {
    ...
    Promise.all([listTeams(), listMeetings()])
      .then(([teamResult, meetingResult]) => {
        ...
        setSelectedTeamId(teamResult.teams.find((team) => team.id === requestedTeamId)?.id ?? teamResult.teams[0]?.id ?? "");
      })
      .catch((cause) => active && setError(cause instanceof Error ? cause.message : "無法讀取工作區資料。"))
      .finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [loadAttempt, requestedTeamId]);
  ...
  async function submitWorkspace(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice(invitations.length ? "團隊已建立，對方登入後可在站內接受邀請，不會寄送 Email。" : "團隊已建立。");
    } catch (cause) {
  ...
  async function removeMember(member: TeamMember) {
    if (!selectedTeamId || !window.confirm(`確定移除 ${memberName(member)}？`)) return;
