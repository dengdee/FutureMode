...
import { type InvitationUser, createInvitation, createTeam, listTeamMembers, listTeams } from "../../lib/api/teams";
...
export default function TeamsPage() {
  ...
  async function loadTeams() {
    ...
    try {
      ...
      const counts = await Promise.all(
        result.teams.map(async (team) => [team.id, (await listTeamMembers(team.id)).members.length] as const),
      );
  ...
  useEffect(() => { void Promise.resolve().then(loadTeams); }, []);
  ...
  async function submitTeam(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!createdTeam) {
        ...
        setMemberCounts((current) => ({ ...current, [team.id]: 0 }));
      }
