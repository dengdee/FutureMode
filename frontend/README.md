...
| UI 元件庫 | 尚未安裝或實作 shadcn/ui、Radix 或其他元件庫 |
| 狀態管理 | 尚未使用 Zustand、React Context 或其他全域狀態方案 |
| 資料請求 | 尚未有 API Client；`app/page.tsx` 只讀取 `NEXT_PUBLIC_API_BASE_URL` 並顯示其值 |
| 現有頁面 | 僅有 `/`，作為前端 scaffold 健康頁 |
| 已完成功能 | Next.js 可啟動、TypeScript、ESLint、Tailwind 與 API Base URL 環境變數 |
| 尚未完成 | 登入、路由、產品畫面、表單、Mock、API、WebSocket、Google Meet Add-on、音訊、AI、外部服務整合 |
...
├─ lib/               # 尚未建立；預計放 API、Mock、formatters
...
| REST／WebSocket schema | 前後端契約 | 尚未建立；先由 `frontend/types/api.ts` 與 Mock adapter 暫定，交給後端確認後再改成正式契約。 |
...
- Mock 與正式 API 必須在 `lib/data/` 或等效 adapter 邊界切換；不得在頁面元件內散落 `if (mock)`。
...
### 步驟 03｜前端 domain 型別、Mock adapter 與資料狀態

- **目標**：在 UI 開發前建立可替換的資料邊界，讓 Mock 與正式 API 不污染元件。
- **使用者情境**：開發者可切換 fixture，驗證空會議、AI 建議、不同角色與錯誤狀態。
- **功能內容**：定義 `User`、`Team`、`Meeting`、`AgendaItem`、`MeetingState`、`AiSuggestion`、`PersonalMessage`、`Decision`、`ActionItem`、`ConsensusVersion` 的前端型別；建立 repository／adapter 介面與 fixture。
- **頁面／路由**：無特定路由，供全站使用。
- **元件規劃**：無產品元件；可有 `DataState`、`MockProvider` 開發工具。
- **狀態管理**：先以 React state／async function 介面；是否採 Zustand 待批准。
- **資料來源**：Mock Data 與 Mock API；每筆 fixture 使用符合預期正式 response 的 `id`、時間、角色與狀態欄位。
- **API 串接**：無正式呼叫；先定義 `getCurrentUser()`、`listMeetings()`、`getMeeting(id)` 等 adapter method。
- **畫面狀態**：mock loading、空資料、可預期錯誤、unauthorized fixture。
- **互動細節**：開發期 fixture 選擇不得出現在正式 production UI。
- **響應式需求**：無。
- **預計異動範圍**：`types/`、`lib/data/`、`lib/mocks/`、必要的環境 feature flag 文件。
- **前置條件**：步驟 01。
- **驗證方式**：TypeScript typecheck；以 fixture 驗證不同狀態可被頁面讀取。
- **完成標準**：後續頁面不直接 import JSON fixture，也不直接呼叫尚未存在的 endpoint。
...
- **資料來源**：Mock adapter。
- **API 串接**：預期 `GET /v1/meetings?scope=upcoming`、`GET /v1/consensus?status=awaiting_response`、`GET /v1/action-items?assignee=me`；response schema 待確認。
- **畫面狀態**：初次 loading、無會議、查詢失敗、無權限團隊、部分區塊失敗。
...
- **完成標準**：使用者可由 Dashboard 進入新建、準備或回顧，不需要知道內部路由。
...
- **資料來源**：Mock team members 與 Mock meeting create adapter。
...
- **完成標準**：可用 Mock 完整建立一場會議並進入其 Prepare 頁；所有欄位有明確驗證與錯誤訊息。
...
- **資料來源**：Mock adapter；Sidekick 回覆使用可重播 fixture，不偽裝為真 AI。
...
- **完成標準**：同一 meeting 的 Sidekick thread 可以在 Mock 下被讀取、發送、整理與經確認公開。
...
- **資料來源**：瀏覽器 Permissions／Mock capture status。
...
- **前置條件**：步驟 03、07。
- **驗證方式**：Mock permission state、不同瀏覽器 user-agent 提示、開始／停止／斷線 UI。
...
- **資料來源**：Mock event timeline，模擬 `ai_suggestion:new`、票數更新與失敗。
...
- **預計異動範圍**：`features/live-state/`、`features/intervention/`、`components/meeting-addon/`、Mock event adapter。
- **前置條件**：步驟 03、04、09。
- **驗證方式**：Member／Host fixture、所有票數／門檻情境、錯誤重試、鍵盤投票與屏讀文字。
- **完成標準**：使用 Mock 可演示「AI 建議 → 投票 → 達標／未達標 → 狀態回饋」全流程。
...
- **目標**：在後端提供並確認契約後，將各 feature 從 Mock adapter 逐項改接正式 REST 與即時事件，而不重寫 UI。
...
- **資料來源**：正式 API；Mock adapter 保留給開發與視覺回歸測試。
...
- **驗證方式**：Mock／正式模式切換、斷線重連、事件重播、兩個瀏覽器視窗同步。
...
- 目前 API 幾乎不存在：所有前端產品功能必須先以 Mock adapter 開發，並保留正式切換邊界。
